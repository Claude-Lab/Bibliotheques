...
/**
 * Classe en charge de gérer les constantes temporelles.
 * 
 * @Version Bibliotheques -v1,0
 * @date 24 mai 2020 - 12:39:54
...
public final class DateUtils {
	...
	protected static LocalDate toLocalDate(Object obj) {
		...
		throw new IllegalArgumentException("Unable to convert to LocalDate from unexpected value '" + obj + "' of type "
	...
	protected static LocalTime toLocalTime(Object obj) {
		...
		if (obj instanceof java.sql.Time) {
			...
			long millis = (int) (time.getTime() % DateUtils.MILLISECONDS_PER_SECOND);
			int nanosOfSecond = (int) (millis * DateUtils.NANOSECONDS_PER_MILLISECOND);
		...
		if (obj instanceof java.sql.Timestamp) {
			...
			return LocalTime.of(timestamp.getHours(), timestamp.getMinutes(), timestamp.getSeconds(),
					timestamp.getNanos());
		}
		if (obj instanceof java.util.Date) {
			java.util.Date date = (java.util.Date) obj;
			long millis = (int) (date.getTime() % DateUtils.MILLISECONDS_PER_SECOND);
			int nanosOfSecond = (int) (millis * DateUtils.NANOSECONDS_PER_MILLISECOND);
		...
		throw new IllegalArgumentException("Unable to convert to LocalTime from unexpected value '" + obj + "' of type "
				+ obj.getClass().getName());
	}
	...
	protected static LocalDateTime toLocalDateTime(Object obj) {
		...
		if (obj instanceof java.util.Date) {
			...
			long millis = (int) (date.getTime() % DateUtils.MILLISECONDS_PER_SECOND);
			if (millis < 0) {
				millis = DateUtils.MILLISECONDS_PER_SECOND + millis;
			}
			int nanosOfSecond = (int) (millis * DateUtils.NANOSECONDS_PER_MILLISECOND);
			return LocalDateTime.of(date.getYear() + 1900, date.getMonth() + 1, date.getDate(), date.getHours(),
					date.getMinutes(), date.getSeconds(), nanosOfSecond);
		}
		throw new IllegalArgumentException("Unable to convert to LocalTime from unexpected value '" + obj + "' of type "
				+ obj.getClass().getName());
	}
